...
<aside class="sidebar">
  ...
  <div class="sidebar-footer">
    ...
    <div style="background:rgba(255,255,255,.07);border-radius:14px;padding:14px;border:1px solid rgba(255,255,255,.08);">
      ...
      <div style="font-size:14px;font-weight:800;color:#d1fae5;">
        <?php echo ($_SESSION['user_role'] === 'admin') ? '⚙️' : '👩‍⚕️'; ?>
        <?php echo htmlspecialchars($adminPrenom); ?>
      </div>
      <div style="font-size:11px;color:rgba(255,255,255,.4);margin-top:4px;text-transform:capitalize;">
...
<main class="main">
  ...
  <section class="panel">
    ...
    <div class="tab-content <?php echo ($onglet === 'utilisateurs') ? 'active' : ''; ?>" id="tab-utilisateurs">
      ...
      <div class="charts-row" style="margin-bottom:24px;">
        <div class="chart-card">
          ...
          <p>Nombre de nouveaux comptes créés chaque mois</p>
          <div class="chart-wrap">
            <canvas id="chartInscriptions"></canvas>
          </div>
          <div style="text-align:center;margin-top:8px;font-size:12px;font-weight:700;color:#688273;" id="inscriptionsTotal"></div>
        </div>
        <div class="chart-card">
    ...
    <div class="tab-content <?php echo ($onglet === 'historique') ? 'active' : ''; ?>" id="tab-historique">
      ...
      <form action="index.php" method="GET" id="formRechHist">
        <input type="hidden" name="page" value="dashboard" />
        <input type="hidden" name="onglet" value="historique" />
        <input type="hidden" name="tri_hist" id="triHistInput"
               value="<?php echo htmlspecialchars($triHist); ?>" />
        <div class="search-bar">
          <div class="search-field">
            <label>Nom</label>
      ...
      <!-- TRI HISTORIQUE -->
      <div class="tri-bar" style="margin-bottom:14px;">
        <span>Trier par :</span>
        <button type="button"
                class="tri-btn <?php echo ($triHist === 'az')     ? 'active' : ''; ?>"
                onclick="setTriHist('az')">🔤 A → Z</button>
        <button type="button"
                class="tri-btn <?php echo ($triHist === 'za')     ? 'active' : ''; ?>"
                onclick="setTriHist('za')">🔤 Z → A</button>
        <button type="button"
                class="tri-btn <?php echo ($triHist === 'recent') ? 'active' : ''; ?>"
                onclick="setTriHist('recent')">🕐 Plus récent</button>
        <button type="button"
                class="tri-btn <?php echo ($triHist === 'ancien') ? 'active' : ''; ?>"
                onclick="setTriHist('ancien')">🕐 Plus ancien</button>
      </div>
...
const inscriptionsTotal  = <?php echo array_sum($inscriptionsParMois); ?>;
...
(function() {
  ...
  // Couleur : mois courant en vert vif, autres en vert clair
  const now = new Date();
  const moisCourantLabel = inscriptionsLabels[inscriptionsLabels.length - 1];
  const bgColors = inscriptionsLabels.map(function(lbl, i) {
    if (i === inscriptionsLabels.length - 1) return 'rgba(23,153,95,1)';   // mois actuel = plein
    const alpha = 0.2 + 0.55 * (inscriptionsData[i] / maxVal);
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            label: function(item) {
              ...
              const suffix = v === 0 ? 'Aucune inscription'
                           : v === 1 ? '1 inscription'
                           : v + ' inscriptions';
              return '  ' + suffix;
            },
            afterLabel: function(item) {
              if (item.dataIndex === inscriptionsLabels.length - 1) {
  ...
  // Afficher le total sous la carte
  var totalEl = document.getElementById('inscriptionsTotal');
  if (totalEl) totalEl.textContent = inscriptionsTotal + ' compte' + (inscriptionsTotal > 1 ? 's' : '') + ' au total';
})();
...
function setTriHist(val) {
  document.getElementById('triHistInput').value = val;
  document.getElementById('formRechHist').submit();
}
