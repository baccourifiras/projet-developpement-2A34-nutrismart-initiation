<?php if (isset($_GET['success'])): ?>
  <?php $m = ['add'=>'Liste ajoutée.','update'=>'Liste modifiée.','delete'=>'Liste supprimée.','duplicate'=>'Liste dupliquée avec succès.']; ?>
    ...
    <tbody>
      ...
      <?php else: ?>
        <?php foreach ($listes as $row):
          ...
          $urlDuplicate = "index.php?page=liste_courses&action=duplicate&id={$row['id']}" . ($space==='back'?'&space=back':'');
        ...
        <tr>
          ...
          <td>
            <div class="actions-group">
              ...
              <a href="<?= $urlDuplicate ?>" class="btn btn-sm"
                 style="background:rgba(124,58,237,.08);color:#7c3aed;border:1px solid rgba(124,58,237,.2);"
                 onclick="return confirm('Dupliquer cette liste ?')">📋 Dupliquer</a>
